...
/**
 * @Class Name : KisaSha256.java
 * @Description : KisaSha256 암호화 처리 - Dreams 제공
 ...
 * </pre>
 */
public class KisaSha256 {

	// DEFAULT : JAVA = BIG_ENDIAN
	private static int ENDIAN = Common256.BIG_ENDIAN;
	...
	private static final int ROTL_ULONG(int x, int n) {
		return (x << n) | Common256.URShift(x, 32 - n);
	}

	private static final int ROTR_ULONG(int x, int n) {
		return Common256.URShift(x, n) | (x << (32 - (n)));
	}
	...
	private static final void BIG_D2B(int D, byte[] B, int B_offset) {
		Common256.int_to_byte_unit(B, B_offset, D, ENDIAN);
	}

	private static final int RR(int x, int n) { return ROTR_ULONG(x, n); }
	private static final int SS(int x, int n) { return Common256.URShift(x, n); }
	...
	private static final void FF(int[] abcdefgh, int a, int b, int c, int d, int e, int f, int g, int h, int[] X, int j) {
		...
		T1 = Common256.intToUnsigned(abcdefgh[h]) + Common256.intToUnsigned(Sigma1(abcdefgh[e])) + Common256.intToUnsigned(Ch(abcdefgh[e], abcdefgh[f], abcdefgh[g])) + Common256.intToUnsigned(SHA256_K[j]) + Common256.intToUnsigned(X[j]);
		abcdefgh[d] += T1;
		abcdefgh[h] = (int)(T1 + Common256.intToUnsigned(Sigma0(abcdefgh[a])) + Common256.intToUnsigned(Maj(abcdefgh[a], abcdefgh[b], abcdefgh[c])));
	}

	private static final int GetData(byte[] x, int x_offset) {
		return Common256.byte_to_int(x, x_offset, ENDIAN);
	}
	/**	
	//*********************************************************************************************************************************
	...
	private static void SHA256_Transform(byte[] Message, int[] ChainVar) {
		...
		for (j = 16; j < 64; j++)
			X[j] = (int)(Common256.intToUnsigned(RHO1(X[j - 2])) + Common256.intToUnsigned(X[j - 7]) + Common256.intToUnsigned(RHO0(X[j - 15])) + Common256.intToUnsigned(X[j - 16]));
	...
	*/
	public static void SHA256_Process( SHA256_INFO Info, byte[] pszMessage, int uDataLen ) {
		...
		Info.uHighLength += Common256.URShift(uDataLen,29);

		pszMessage_offset = 0;
		while (uDataLen >= SHA256_DIGEST_BLOCKLEN) {
			Common256.arraycopy_offset(Info.szBuffer, 0, pszMessage, pszMessage_offset, SHA256_DIGEST_BLOCKLEN);
			SHA256_Transform(Info.szBuffer, Info.uChainVar);
			pszMessage_offset += SHA256_DIGEST_BLOCKLEN;
			uDataLen -= SHA256_DIGEST_BLOCKLEN;
		}

		Common256.arraycopy_offset(Info.szBuffer, 0, pszMessage, pszMessage_offset, uDataLen);
	}
	...
	*/
	public static void SHA256_Close( SHA256_INFO Info, byte[] pszDigest ) {
		...
		Index = Common256.URShift(Info.uLowLength, 3) % SHA256_DIGEST_BLOCKLEN;
		Info.szBuffer[Index++] = (byte)0x80;

		if (Index > SHA256_DIGEST_BLOCKLEN - 8) {
			Common256.arrayinit_offset(Info.szBuffer, Index, (byte)0, SHA256_DIGEST_BLOCKLEN - Index);
			SHA256_Transform(Info.szBuffer, Info.uChainVar);
			Common256.arrayinit(Info.szBuffer, (byte)0, SHA256_DIGEST_BLOCKLEN - 8);
		}
		else {
			Common256.arrayinit_offset(Info.szBuffer, Index, (byte)0, SHA256_DIGEST_BLOCKLEN - Index - 8);
		}

		if(ENDIAN == Common256.LITTLE_ENDIAN) {
			Info.uLowLength = ENDIAN_REVERSE_ULONG(Info.uLowLength);
			Info.uHighLength = ENDIAN_REVERSE_ULONG(Info.uHighLength);
		}

		Common256.int_to_byte_unit(Info.szBuffer, ((int)(SHA256_DIGEST_BLOCKLEN / 4 - 2))*4, Info.uHighLength, ENDIAN);
		Common256.int_to_byte_unit(Info.szBuffer, ((int)(SHA256_DIGEST_BLOCKLEN / 4 - 1))*4, Info.uLowLength, ENDIAN);
...
/*	
	...
	*/
	...
	public byte[] Sha256EncryptB(byte[] plainTextByte){
